...
def smooth_logits_gnn(attr_idx, edge_idx, model, sample_config, n, d, nc, batch_size=1, idx_nodes=None):
    ...
    for _ in range(nbatches):
        ...
        x = torch.zeros(n, d)
        x[attr_idx_batch[0], attr_idx_batch[1]] = 1
        x = x.cuda()
        logits_batch = model(x=x, edge_index=edge_idx_batch)
        if mean_softmax:
...
def train_gnn(model, edge_idx, attr_idx, labels, n, d, nc,
    ...
    for it in range(max_epochs):
        # if sample_config is not None (randomized augmentation), sample then train
        # else train directly
